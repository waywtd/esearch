...
public class DateUtil
{
    ...
    /**
     * [11/Jul/2016:00:03:42 +0000]转为11/Jul/2016:00:03:42 +0000
     * 
     * @Title: getTime
     * @Description: TODO(截取时间)
     * @param @return
    ...
    {
        Matcher tiMatcher = CommonUtil.getMatcher(time, Constant.timeMatcher);
        // 获取时间
        while (tiMatcher.find())
        {
            time = tiMatcher.group();

        }
        return time;
    }
    
    public static String convertDate(String timeUtc, String pattern)
    {
        String stDate = null;
        if (StringUtils.isNotEmpty(timeUtc))
        {
            try
            {
                String timeStr = timeUtc.substring(1, timeUtc.length() - 1);
                String str = timeStr.split(":")[0];
                SimpleDateFormat sdf = new SimpleDateFormat("dd/MMM/yyyy", Locale.US);
                SimpleDateFormat sdf1 = new SimpleDateFormat(pattern, Locale.US);
                long time = sdf.parse(str).getTime();
                Date date = new Date(time);
                stDate = sdf1.format(date);
            }
            catch (ParseException e)
            {
                e.printStackTrace();
    ...
    {
        Date date=null;
        String parent="yyyy-MM-dd";
        String dateOfBirth="2017-10-20";
        SimpleDateFormat sdf=new SimpleDateFormat(parent);
        try
        {
            date=sdf.parse(dateOfBirth);
            Date date01=DateUtil.addDate(date,0,0,100,0,0,0,0);
            String Day100=sdf.format(date01);
