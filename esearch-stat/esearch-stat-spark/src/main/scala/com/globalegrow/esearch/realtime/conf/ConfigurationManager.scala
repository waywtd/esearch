package com.globalegrow.esearch.realtime.conf
...
/**
  ...
  *
  */
object ConfigurationManager { // Properties对象使用private来修饰，就代表了其是类私有的
  ...
  def getAllProperties() :Properties = prop
  ...
  try try { // 通过一个“类名.class”的方式，就可以获取到这个类在JVM中对应的Class对象
    ...
    val in = classOf[ConfigurationManager].getClassLoader.getResourceAsStream("realtime.properties")
